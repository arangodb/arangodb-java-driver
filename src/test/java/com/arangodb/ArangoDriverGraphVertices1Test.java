...
public class ArangoDriverGraphVertices1Test extends BaseGraphTest {
  ...
  // TestComplexEntity01("user" + i, "desc" + i, (i/10)+1), null);
  // assertThat(v1.getCode(), is(202));
  // }
  //
  // CursorResultSet<DocumentEntity<TestComplexEntity01>> rs =
  // driver.getVerticesWithResultSet("g1", TestComplexEntity01.class);
  ...
  //
  // CursorResultSet<DocumentEntity<TestComplexEntity01>> rs =
  // driver.getVerticesWithResultSet("g1", TestComplexEntity01.class, null,
  // null, true);
  ...
  // driver.getVertices("g1", TestComplexEntity01.class, batchSize, limit,
  // count);
  ...
  // assertThat(cursor.getResults().get(0).getEntity(),
  // instanceOf(TestComplexEntity01.class));
  // assertThat(cursor.getResults().get(0).getEntity().getUser(),
  // is(notNullValue()));
  // assertThat(cursor.getResults().get(0).getEntity().getDesc(),
  // is(notNullValue()));
  ...
  // Boolean count = true;
  // CursorResultSet<DocumentEntity<TestComplexEntity01>> rs =
  // driver.getVerticesWithResultSet("g1", TestComplexEntity01.class, batchSize,
  // limit, count);
  //
  // int i = 0;
  // while (rs.hasNext()) {
  ...
  //
  // // next round trip
  // CursorEntity<DocumentEntity<TestComplexEntity01>> cursor2 =
  // driver.continueQuery(cursor.getCursorId(), DocumentEntity.class,
  ...
  // assertThat(cursor2.getResults().get(0).getEntity(),
  // instanceOf(TestComplexEntity01.class));
  // assertThat(cursor2.getResults().get(0).getEntity().getUser(),
  // is(notNullValue()));
  // assertThat(cursor2.getResults().get(0).getEntity().getDesc(),
  // is(notNullValue()));
  ...
  // for (int i = 1; i <= 100; i++) {
  // DocumentEntity<TestComplexEntity01> v1 = driver.createVertex("g1", new
  // TestComplexEntity01("user" + i, "desc" + (i%10), (i/10)+1), null);
  // assertThat(v1.getCode(), is(202));
  // }
  //
  // Integer batchSize = 2;
