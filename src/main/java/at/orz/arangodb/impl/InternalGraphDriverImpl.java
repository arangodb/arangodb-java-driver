...
public class InternalGraphDriverImpl extends BaseArangoDriverWithCursorImpl implements at.orz.arangodb.InternalGraphDriver {
    ...
    Boolean waitForSync
		...
		HttpResponseEntity response = httpManager.doPost(
				...
				EntityFactory.toJsonString(
						new MapBuilder()
						.put("name", graphName)
						.put("edgeDefinitions", edgeDefinitions)
						.put("orphanCollections", orphanCollections)
						.get()));
		return createEntity(response, GraphEntity.class);
	}
    ...
    Boolean waitForSync
		...
		HttpResponseEntity res = httpManager.doPost(
				...
				EntityFactory.toJsonString(new MapBuilder()
						.put("_key", documentKey).put("vertices", vertices)
						.put("edges", edges).get()));
  ...
  public <T> DocumentEntity<T> getVertex(String database, String graphName,
		...
		HttpResponseEntity res = httpManager.doGet(
				...
				new MapBuilder()
						.put("If-None-Match", ifNoneMatchRevision, true)
						.put("If-Match", ifMatchRevision, true).get(),
  ...
  public DeletedEntity deleteVertex(String database, String graphName,
		...
		HttpResponseEntity res = httpManager.doDelete(
				createEndpointUrl(baseUrl, database, "/_api/graph",
						...
						StringUtils.encodeUrl(key)),
				new MapBuilder().put("If-Match", ifMatchRevision, true).get(),
				new MapBuilder().put("waitForSync", waitForSync)
						.put("rev", rev).get());
  ...
  public <T> DocumentEntity<T> updateVertex(String database,
		...
		HttpResponseEntity res = httpManager.doPatch(
				createEndpointUrl(baseUrl, database, "/_api/graph",
						...
						StringUtils.encodeUrl(key)),
				new MapBuilder().put("If-Match", ifMatchRevision, true).get(),
				new MapBuilder().put("keepNull", keepNull)
						.put("waitForSync", waitForSync).put("rev", rev).get(),
				EntityFactory.toJsonString(vertex, keepNull != null
						&& !keepNull));
  ...
  public <T> CursorEntity<DocumentEntity<T>> getVertices(String database,
		...
		Map<String, Object> filter = new MapBuilder()
				.put("direction", toLower(direction)).put("labels", labels)
				.put("properties", properties).get();

		HttpResponseEntity res = httpManager.doPost(
				createEndpointUrl(baseUrl, database, "/_api/graph",
						StringUtils.encodeUrl(graphName), "vertices",
						StringUtils.encodeUrl(vertexKey)), null, EntityFactory
						.toJsonString(new MapBuilder()
								.put("batchSize", batchSize)
								.put("limit", limit).put("count", count)
								.put("filter", filter).get()));

		return createEntity(res, CursorEntity.class, DocumentEntity.class,
				clazz);
  ...
  public <T> CursorResultSet<DocumentEntity<T>> getVerticesWithResultSet(
    ...
    FilterCondition... properties) throws ArangoException {
		...
		CursorEntity<DocumentEntity<T>> entity = getVertices(database,
				...
				direction, labels, properties);
		CursorResultSet<DocumentEntity<T>> rs = new CursorResultSet<DocumentEntity<T>>(
				database, cursorDriver, entity, DocumentEntity.class, clazz);
		return rs;
	}
  ...
  public <T> EdgeEntity<T> createEdge(String database, String graphName,
		...
		if (value == null) {
			...
			if (elem.isJsonObject()) {
				...
				throw new IllegalArgumentException(
						"value need object type(not support array, primitive, etc..).");
			}
		}
		...
		return createEntity(res, EdgeEntity.class,
  ...
  public <T> EdgeEntity<T> replaceEdge(String database, String graphName,
		...
		HttpResponseEntity res = httpManager.doPut(
				...
				new MapBuilder().put("If-Match", ifMatchRevision, true).get(),
				new MapBuilder().put("waitForSync", waitForSync)
						.put("rev", rev).get(), value == null ? null
  ...
  public <T> CursorEntity<EdgeEntity<T>> getEdges(String database,
		...
		HttpResponseEntity res = httpManager.doPost(
				createEndpointUrl(baseUrl, database, "/_api/graph",
						StringUtils.encodeUrl(graphName), "edges",
						StringUtils.encodeUrl(vertexKey)),
				null,
				EntityFactory.toJsonString(new MapBuilder()
						.put("batchSize", batchSize).put("limit", limit)
  ...
  public <T> CursorResultSet<EdgeEntity<T>> getEdgesWithResultSet(
    ...
    FilterCondition... properties) throws ArangoException {
		...
		CursorEntity<EdgeEntity<T>> entity = getEdges(database, graphName,
				...
				properties);
		CursorResultSet<EdgeEntity<T>> rs = new CursorResultSet<EdgeEntity<T>>(
				database, cursorDriver, entity, EdgeEntity.class, clazz);
		return rs;
	}
